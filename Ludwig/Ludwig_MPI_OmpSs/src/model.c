 ...
 *  relevant data rather than MPI data types. It is therefore a 'full'
...
#pragma omp taskloop num_tasks(8)
      for (jc = 1; jc <= nlocal[Y]; jc++) {
        for (kc = 1; kc <= nlocal[Z]; kc++) {
          ...
          count = (kc-1) + nlocal[Z] * ((jc-1) + nlocal[Y] * (n + lb->ndist * p));
...
#pragma omp task out(fakeX)
  {
    ...
    if (cart_size(X) == 1) {
      ...
      /* MPI_Issend(sendback, nsend, MPI_DOUBLE, pback, tagb, comm, reqSend); */
      /* MPI_Issend(sendforw, nsend, MPI_DOUBLE, pforw, tagf, comm, reqSend+1); */
...
#pragma omp taskloop num_tasks(8)
      for (ic = 0; ic <= nlocal[X] + 1; ic++) {
        for (kc = 1; kc <= nlocal[Z]; kc++) {
          ...
          count = (kc-1) + nlocal[Z] * (ic + (nlocal[X]+2) * (n + lb->ndist * p));
...
#pragma omp taskloop num_tasks(8)
      for (ic = 0; ic <= nlocal[X] + 1; ic++) {
        for (jc = 0; jc <= nlocal[Y] + 1; jc++) {
          ...
          count = (jc) + (nlocal[Y]+2) * (ic + (nlocal[X]+2) * (n + lb->ndist * p));
...
int lb_halo_via_copy_nonblocking_end(lb_t * lb) {
  ...
  int a;
  //  for (n=0; n<26; n++){
  //#pragma omp task inout(lb)
  MPI_Waitall(26, lb->hl.recvreq, lb->hl.status);
  // }

  /* Copy data from MPI buffers */
  //#pragma omp task in(a) out(lb)
  unpack_halo_buffers(lb);

  //  for (n=0; n<26; n++){
  //    MPI_Waitany(26, lb->hl.sendreq, &sendcount, lb->hl.status);
  //#pragma omp task in(lb)
  MPI_Waitall(26, lb->hl.sendreq, lb->hl.status);
  //}
