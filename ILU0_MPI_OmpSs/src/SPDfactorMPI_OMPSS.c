...
ptr_ILU0Factor InitILU0FactorizationMPIOMPSS (SparseMatrix spr, int index, int nleaves,
																								Ilpck_Comm ilpkcomms, char *matrix) {
	// Definition of the local variables
	int i, j, dim = spr.dim1, dimL, dimT;
...
#ifdef CHANGE_ORDER
// NEW CODE FOR CHANGE THE ORDERING OF THE TREETAB VECTOR IF WE USE PARMETIS 
	if (my_id == root) {
		if ( nleaves > 2 ){
			int *treeN = NULL, *rangN = NULL, *permT = NULL, *ipermT = NULL, *permN = NULL;
			int lev = log2(nleaves)+1, k;

			CreateInts (&treeN, dimT); CreateInts (&rangN , dimT); CreateInts (&permN, dim);
			CreateInts (&permT, dimT); CreateInts (&ipermT, dimT); 
			PostOrderOrdering(permT, treeN, dimT, lev); 
			ComputeInvPermutation (permT, ipermT, 0, dimT-1);
			printf ("PERMS\n"); PrintInts (permT, dimT); PrintInts (ipermT, dimT);
			rangN[0] = 0;
			for (i=0; i<(dimT-1); i++) {
				k = rangtab[ipermT[i]+1] - rangtab[ipermT[i]];
				rangN[i+1] = rangN[i] + k;
				CopyInts (perm+rangtab[ipermT[i]], permN+rangN[i], k);
			}
			printf ("TREES\n"); PrintInts (treetab, dimT); PrintInts (treeN, dimT);
			printf ("RANGS\n"); PrintInts (rangtab, dimT); PrintInts (rangN, dimT);
			RemoveInts (&ipermT); RemoveInts (&permT); 
			CopyInts (permN, perm   , dim   ); RemoveInts (&permN); 
			CopyInts (rangN, rangtab, dimT  ); RemoveInts (&rangN); 
			CopyInts (treeN, treetab, dimT-1); RemoveInts (&treeN); 
			printf ("TREE\n"); PrintInts (treetab, dimT);
			printf ("RANG\n"); PrintInts (rangtab, dimT);
			printf ("PERM\n"); PrintInts (perm, dimT);
...
#ifdef VERBOSE
		if (my_id == root) {
			printf ("sorttab --> "); PrintFInts (sorttab, dimL, 10, 0);
		}
#endif
...
int ILU0FactorizationMPIOMPSS (SparseMatrix spr, int index, int nleaves, paramFactor parFac, 
  ...
  MPI_Barrier (comm); reloj (&tge1, &tgu1); tt1 = MPI_Wtime ();
	vFact = InitILU0FactorizationMPIOMPSS (spr, index, nleaves, ilpkcomms, matrix);
  mTab = vFact->mTab; tGlb = vFact->tGlb;
  reloj (&tge2, &tgu2); tt2 = MPI_Wtime ();
  tGlb[0][TGFMET] = tge2-tge1; tGlb[1][TGFMET] = tgu2-tgu1; tt = tt2 - tt1;
  ...
  if (ilpkcomms.color) {
    ...
    ownrtab = mTab[OWNR]; hgthtab = mTab[HGTH]; v_ord = vFact->mTab[LEV];
#ifdef VERBOSE
    if (my_id == 0) printf ("(BEGIN) ILU0LeavesDistribution\n");
#endif
    ILU0LeavesDistribution_OMPSS (spr, index, nleaves, parFac, ilpkcomms, vFact);
#ifdef VERBOSE
    ...
    for (i = 0; i < remaining_tsks; i++){
			#pragma oss task in (chldtab[dependencies[i][0]], chldtab[dependencies[i][1]]) out (chldtab[i])
			{
  ...
  printf ("Preconditioner Time (%d)= (%20.15e,%20.15e)\n", my_id, tge2-tge1, tgu2-tgu1);
  // Print ownrtab
