...
#ifdef ORDER_MESSAGES
int order;
#pragma oss task in(*sendbuf) out(*recvbuf) inout(order) label(forward_particles_block)
#else
#pragma oss task in(*sendbuf) out(*recvbuf) label(forward_particles_block)
#endif
