...
// #define VERBOSE 1
// #define READ_DATAMETIS 1
// #define WRITE_DATAMETIS 1 
...
// Create the structures which are required for the resolution of the system
extern void InitTaskQueuesResolutionMPI (ptr_ILU0Factor vFact, int nthreads);
...
extern void InitVectorResolutionMPIOPENMP (ptr_ILU0Factor vFact);
...
// The name of the text files are obtained joining fname 
// to the number of the task.
extern void WriteVectorMPI (ptr_ILU0Factor vFact, int vid, char *fname);

// This routine writes in files the vector vid of the leaves.
// The name of the text files are obtained joining fname 
// to the number of the task.
extern void WriteVectorMPIOPENMP (ptr_ILU0Factor vFact, int vid, char *fname);

// This routine reads	in the vector vid of the leaves the files.
// The name of the text files are obtained joining fname 
// to the number of the task.
extern void ReadVectorMPI (ptr_ILU0Factor vFact, int vid, char *fname);

// This routine reads	in the vector vid of the leaves the files.
// The name of the text files are obtained joining fname 
// to the number of the task.
extern void ReadVectorMPIOPENMP (ptr_ILU0Factor vFact, int vid, char *fname);

// This routine copies the vector vidI to the vector vidO
extern void CopyVectorsMPI (ptr_ILU0Factor vFact, int vidI, int vidO);

extern void InitResolutionMPI (ptr_ILU0Factor vFact, double *vec, int vid, int createVect, 
												int nleaves, int adjustVect, Ilpck_Comm ilpkcomms);

extern void InitResolutionMPIOPENMP (ptr_ILU0Factor vFact, double *vec, int vid, int createVect, 
												int nleaves, int adjustVect, Ilpck_Comm ilpkcomms);

extern void CloseResolutionMPI (ptr_ILU0Factor vFact, int nleaves, int vid, double *sol, 
...
extern void ComputeProductMPI (ptr_ILU0Factor vFact, int vid1, int vid2);

// #define ERROR_REDUCTION 1
// Computation of the vector operations (optV) related to the processor/hebra id,
// on the data included in the vector of preconditioners.
// The parameters vid1 and vid2 identify the vectors on which the operation will be made.
extern double ComputeVectorOperationsMPIOPENMP (ptr_ILU0Factor vFact, int optV, int vid1,
...
extern void ResolutionTransformMPI (ptr_ILU0Factor vFact, int nleaves, int vidI, int vidO1, int vidO2, 
															Ilpck_Comm ilpkcomms);
...
// Each thread uses a different queue, exploiting the locality.
// The indices vid's specify where the different auxiliar vector is.
extern void ResolutionTransformMPIOPENMP (ptr_ILU0Factor vFact, int nleaves, int vidI, int vidO1, 
