...
void ILU0LeavesDistribution_OMPSS (SparseMatrix spr, int index, int nleaves, paramFactor parFac,
  ...
  int i, j, dim = spr.dim1, dimL;
  int *perm = NULL, *rangtab = NULL, *treetab = NULL, dimT;
  int *sizetab = NULL, *chldtab = NULL, *nmchtab = NULL, *brthtab = NULL, *wgthtab = NULL, *v_ord = NULL;
  int remaining_tsks, *marktab = NULL, *ownrtab = NULL, *hgthtab = NULL, *sorttab = NULL;
  // Definition of the local vectors and variables
  matDoubles tGlb = NULL;
  matInts mTab;
  int tsk = -1, chld = -1, vint[6];
  int my_id, numprocs, root, nleavespr;
  int indexM = 0, ierr = 0; 
  MPI_Comm comm;
  double tle1, tle2, tlu1, tlu2, tt1, tt2, tt;
  SortType_func sort = NULL;
  Seek_func seek = NULL;
  MPI_Status st;
  int proc, num, tid = 0;
  ...
  dimL = vFact->dimL; dimT = vFact->dimT; perm = vFact->perm;
  mTab = vFact->mTab; tGlb = vFact->tGlb;
  rangtab = mTab[RANG]; treetab = mTab[TREE];
  chldtab = mTab[CHLD]; nmchtab = mTab[NMCH]; brthtab = mTab[BRTH]; wgthtab = mTab[WGTH];
  marktab = mTab[MARK]; ownrtab = mTab[OWNR]; hgthtab = mTab[HGTH]; sizetab = mTab[SIZE];
  sorttab = mTab[SORT], v_ord = mTab[LEV];;

  if (ilpkcomms.color) {
    MPI_Barrier (comm); reloj (&tle1, &tlu1); tt1 = MPI_Wtime ();
    // Initialize the queue of active nodes with the leaves of the tree
    sort = OrderUpSortType; seek = SeekChildren;
    sort = NULL           ; seek = NULL        ;
    sort = OrderUpSortType; seek = NULL        ;
...
void ILU0LeavesDistributionPCG_OMPSS (SparseMatrix sprR, int index, int nleaves, paramFactor parFac, Ilpck_Comm ilpkcomms, ptr_ILU0Factor vFact) {
  ...
  // Definition of the global vectors and variables
  int i,j, dim = sprR.dim1, dimL;
  int *perm = NULL, *rangtab = NULL, *treetab = NULL, dimT, *v_ord = NULL;
...
double ComputeVectorOperationsOMPSS (ptr_ILU0Factor vFact, int tid, 
  ...
  for(tsk=0; tsk<dimL; tsk++){
    if(chldtab[tsk] == -1 && ownrtab[tsk] >= 0){
      ...
      prio=0;  
    #endif
			#pragma omp task concurrent(res_gbl) priority(prio)  
      {
        double te1, tu1, te2, tu2;
        double res = 0.0;
        reloj (&te1, &tu1);
...
// The parameters vid1 and vid2 identify the vectors on which the operation will be made.
void ComputeProductOMPSS (ptr_ILU0Factor vFact, int tid, 
											int vid1, int vid2) {
	// Definition of the local variables
	int tsk;
	double *ptr1 = NULL, *ptr2 = NULL;
	...
	int dimL = vFact->dimL;
  int *chldtab = vFact->mTab[CHLD], *wgthtab = vFact->mTab[WGTH];
	int *ownrtab = vFact->mTab[OWNR];
  int prio, nleaves = (dimL+1)/2;
